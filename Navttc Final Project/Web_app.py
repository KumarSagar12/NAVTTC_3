...
d_data= pd.read_excel("diabetes_data.xlsx")

options=st.sidebar.radio("Options",["Introduction","Exploratory Data Analysis","Data Visualization","Scaling the Data","Model Building"])
...
def EDA():
    ...
    if st.checkbox("Imputing the Missing Values"):
        ...
        if st.checkbox("Dataset After Imputation"):
            ...
            rows = st.number_input("", min_value=0,value=5)
            if rows > 0:
                st.table(d_data_copy.head(rows))
